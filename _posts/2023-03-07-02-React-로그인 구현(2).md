...
- `axios.get("http://localhost:4000/userinfo").the(...)`
  - 서버에 정보를 요청하고 전달받은 정보를 userInfo 상탯값에 담는다.
...
- `Login({setIsLogin, setUserInfo})`
  - App.js 로부터 상탯값을 내려받는다.
- `const [loginInfo, setLoginInfo] = useState({...})`
  - 로그인 정보에 관한 상탯값을 갖고 있으며, 초기값은 빈 문자열로 둔다.
- `const [checkedKeepLogin, setCheckedKeepLogin] = useState(false)`
  - 자동 로그인을 지정할 상탯값을 만든다.
...
- `const handleInputValue = (key) => (e) => {setLoginInfo({...loginInfo, [key]: e.target.value})}`
  - input 아이디에서 작성한 값을 전달받아 상탯값의 id에 담는다.
  - input 패스워드도 같은 원리로 작동한다.
- `if(!loginInfo.userId || !loginInfo.password)`
  - 아이디와 패스워드 input을 통해 상탯값이 변경될 건데, id와 password 중 하나라도 입력받지 않았다면 에레 메세지를 내보낸다.
...
- `then(res)=> {setUserInfo(res.data); setIsLogin(true); setErrorMessage("")}`
  ...
  - Login에 대한 상탯값을 true로 바꾸고, 에러 메세지는 빈 문자열로 둔다.
